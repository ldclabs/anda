...
//!
//! This module provides integration with Xiaomi Mimo's API.
...
// ================================================================
// Main Xiaomi Mimo Client https://platform.xiaomimimo.com/#/docs/api/chat/openai-api
// ================================================================
const API_BASE_URL: &str = "https://api.xiaomimimo.com/v1";
pub static MIMO_V2_PRO: &str = "mimo-v2-pro";

/// Xiaomi Mimo API client configuration and HTTP client
#[derive(Clone)]
pub struct Client {
    endpoint: String,
    api_key: String,
...
impl Client {
    /// Creates a new Xiaomi Mimo client instance with the provided API key
    ///
    /// # Arguments
    /// * `api_key` - Xiaomi Mimo API key for authentication
    ///
    /// # Returns
    /// Configured Xiaomi Mimo client instance
    pub fn new(api_key: &str, endpoint: Option<String>) -> Self {
        ...
        Self {
            ...
            http: request_client_builder()
                .build()
                .expect("Mimo reqwest client should build"),
        }
    }
    ...
    /// Creates a new completion model instance using the default Mimo model
    pub fn completion_model(&self, model: &str) -> CompletionModel {
        CompletionModel::new(
            self.clone(),
            if model.is_empty() { MIMO_V2_PRO } else { model },
        )
    }
}

/// Token usage statistics from Mimo API responses
...
/// Completion response from Mimo API
...
/// Individual completion choice from Mimo API
#[derive(Debug, Deserialize, Serialize)]
pub struct Choice {
    pub index: usize,
...
/// Tool call output structure from Mimo API
#[derive(Debug, Deserialize, Serialize)]
...
/// Completion model wrapper for Mimo API
#[derive(Clone)]
pub struct CompletionModel {
    /// Mimo client instance
    client: Client,
    /// Model identifier
    pub model: String,
    // enabled， disabled
    ...
    fn completion(&self, mut req: CompletionRequest) -> BoxPinFut<Result<AgentOutput, BoxError>> {
        ...
        Box::pin(async move {
            ...
            if !req.instructions.is_empty() {
                raw_history.push(serde_json::to_value(MessageInput {
                    role: "system".into(),
            ...
            if let Some(temperature) = req.temperature {
                // Mimo temperature is in range [0, 1.5]
                body.insert("temperature".to_string(), Json::from(temperature.min(1.5)));
            }
            ...
            {
                log::debug!(request = val; "Mimo completions request");
            }
            ...
            if response.status().is_success() {
                ...
                match serde_json::from_str::<CompletionResponse>(&text) {
                    ...
                    Err(err) => {
                        Err(format!("Mimo completions error: {}, body: {}", err, text).into())
                    }
                }
            } else {
                ...
                Err(format!("Mimo completions error: {}", msg).into())
            }
        })
    }
}
