...
Anda フレームワークは `anda_cli` で Anda エンジンサーバーと対話するためのコマンドラインインターフェースを提供します。

#### 開発者向け:

- コアエンジン `anda_core` および `anda_engine` を強化できます。
- `anda_core` のトレイトを使用してカスタムエージェントやツールを構築できます。
- `anda_engine_server` HTTP サーバーの実装に貢献できます。
